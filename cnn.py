...
class PortConfig:
    def __init__(self, usn_suffix):
        try:
            ...
            log("[WARNING] Invalid USN suffix. Using default ports 8832 and 8732.")
            self.http_port = 8832
            self.ftp_port = 8732
...
www IN A 127.0.0.1'''
        with open("/etc/bind/db.mylocal", 'w') as f:
            f.write(zone_file)
        run_cmd("chown bind:bind /etc/bind/db.mylocal")
        run_cmd("chmod 644 /etc/bind/db.mylocal")
...
subnet 192.168.1.0 netmask 255.255.255.0 {
  ...
  option domain-name-servers 8.8.8.8;
        ...
        with open("/etc/dhcp/dhcpd.conf", 'w') as f:
            f.write(dhcp_conf)
        run_cmd('sed -i "s/INTERFACESv4=\"\"/INTERFACESv4=\"eth0\"/" /etc/default/isc-dhcp-server')
...
"""
        ...
        run_cmd(f"ufw allow {self.ports.http_port}/tcp")
        run_cmd(f"ufw allow {self.ports.ftp_port}/tcp")
